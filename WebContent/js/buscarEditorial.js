...
function llamadaANombreEditoriales() {
	...
	if (req){
		...
		req.open("POST", "buscarEditorial.do", true);
		req.inicioPeticion = 0;
		req.send("inicio");
	}
}
...
function funcionAsincronaRecepcionNombre(){
	...
	if (ready == READY_STATE_COMPLETE){
		...
		var tiempoPeticion = array[2]; // tiempo que tarda la peticion
		...
		document.getElementById("pie").innerHTML = tiempoPeticion;
...
function funcionAsincronaRecepcionDatosEditoriales(){
	...
	if (ready == READY_STATE_COMPLETE){
		...
		// Debido a que la respuesta del servidor contine xml y texto, se
		// intercala el separador "#####" para diferenciar uno de otro. Con
		// la función split partimos la respuesta en 2 separaqmos html de
		// xml
		var array = data.split("#####");
		var datosXml = array[0];  // datos xml
		var tiempoPeticion = array[1]; // tiempo que ha tardado la petición
